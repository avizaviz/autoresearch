...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        # Tie input/output embeddings: fewer params, stronger inductive bias for LM
    ...
    def init_weights(self):
        # Embedding and unembedding (shared weight — init once)
    ...
    def num_scaling_params(self):
        ...
        lm_head = (
            0 if self.lm_head.weight is self.transformer.wte.weight else sum(p.numel() for p in self.lm_head.parameters())
        )
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        lm_head_params = (
            [] if self.lm_head.weight is self.transformer.wte.weight else list(self.lm_head.parameters())
        )
        ...
        param_groups = [
            ...
            # Tied lm_head uses embedding_lr only (lm_head_params empty when tied)
...
# Optimization
TOTAL_BATCH_SIZE = 2**19 if DEVICE_TYPE == "cuda" else 2**13  # 524K on CUDA, 8K on MPS
...
DEVICE_BATCH_SIZE = 128 if DEVICE_TYPE == "cuda" else 4  # reduce for MPS memory
...
# Swarm model save hook
import os as _os
_save_path = _os.environ.get("SWARM_SAVE_MODEL")
if _save_path:
    _os.makedirs(_os.path.dirname(_save_path), exist_ok=True)
    torch.save(model, _save_path)
    print(f"model_saved:      {_save_path}")
